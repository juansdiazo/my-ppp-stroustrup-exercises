...
double higher_term()
{
    ...
    while (true) {
        switch (t.kind) {
        case '!':
            if (left<0) error("factorial not defined for negative numbers");
            fact = (int)left;
            if (fact == 0) return fact_result;
            else for (int i=fact; i>0; i--) fact_result*=i;
            return fact_result;
            break; 
        default:
